 ...
 * This HW4.HashingProblems object contains three methods / problems that you must
...
class HashingProblems {
    ...
    public double getAverage(HashMap<Integer, Integer> map, int[] array) {
        ...
        for (int key : array) {
            if (map.containsKey(key)) {
                ...
                if (value != null) { // If the key exists in the map
                    sum += value; // Add the value to the total
                    count++;
    ...
    public ArrayList<String> odd(HashMap<Integer, String> map) {
        ...
        for (HashMap.Entry<Integer, String> entry : map.entrySet()) {
            ...
            if (key % 2 != 0) { // Check if the key is odd
                result.add(entry.getValue()); // Add the value to the result list
            }
        }
    ...
    public int twoSums(int[] numbers, int k) {
        ...
        // First pass: populate the HashSet with elements of the array
        for (int num : numbers) {
            set.add(num);
        }

        // Second pass: check for pairs that satisfy the difference condition
        for (int num : numbers) {
            if (set.contains(num + k)) {
                count++; // Increment if num + k exists in the set
            }
            if (set.contains(num - k)) {
                count++; // Increment if num - k exists in the set
            }
            set.remove(num); // Ensure each pair is only counted once
        }
